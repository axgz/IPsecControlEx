...
if ($Create -or $Update) {    
    $IPsecRules = @(
        ...
        @{
            Protect = $true
            DisplayName = "(test only) RDP (TCP 3389) to any target."
            UniqueId = "RDP Global"
            Protocol = "TCP"
            RemotePort = @(
                "3389"
        ...
        @{
            Protect = $true
            DisplayName = "(test only) WinRM (TCP 5985, 5986) to any target."
        ...
        @{
            Protect = $false
            DisplayName = "DHCP (UDP 67, 68) to any target."
            UniqueId = "DHCP Global"
            Protocol = "UDP"
            RemotePort = @(
                "67",
                "68"
        ...
        @{
            Protect = $false
            DisplayName = "VPN (TCP 443) to Azure P2S Gateway Public IP."
            UniqueId = "VPN Global"
            Protocol = "TCP"
            RemotePort = @(
                "443"
...
if (-not $UseLocalPersistentStore) {
    if ($config.AppSettings.ClientGpoId -eq '') {
        $gpo = Get-GPO -All | Out-GridView -OutputMode Single -Title "Select the 'Client' IPsec Policy GPO"
        $config.AppSettings.ClientGpoId = $gpo.Id.Guid
    }
    else {
        $gpo = Get-GPO -Id $config.AppSettings.ClientGpoId
    }
}
